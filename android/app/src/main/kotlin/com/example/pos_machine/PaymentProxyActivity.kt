...
class PaymentProxyActivity : Activity() {
    ...
    private fun startLoginFlow() {
        ...
        if (packageName.isEmpty() || userName.isEmpty()) {
            deliverAndFinish(
                JSONObject()
                    .put("stage", "login")
                    .put("status", "failed")
                    .put("message", "Missing package name or terminal ID for payment.")
                    .put("launchSurface", "front")
                    .toString(),
            )
            return
        }
        ...
        try {
            ...
            Log.i(TAG, "Started Mosambee login from front proxy activity")
    ...
    private fun handleLoginResult(resultCode: Int, data: Intent?) {
        ...
        try {
            ...
            Log.i(TAG, "Started Mosambee payment from front proxy activity")
        } catch (error: ActivityNotFoundException) {
            deliverAndFinish(
                JSONObject()
                    .put("stage", "payment")
            ...
            deliverAndFinish(
                JSONObject()
                    .put("stage", "payment")
                    .put("status", "failed")
                    .put("message", "Unable to continue payment.")
                    .put("error", error.toString())
                    .put("launchSurface", "front")
                    .toString(),
            )
    ...
    companion object {
        private const val TAG = "PaymentProxy"
        private const val ACTION_LOGIN = "com.mosambee.softpos.login"
        private const val ACTION_PAYMENT = "com.mosambee.softpos.payment"
        private const val LOGIN_REQUEST_CODE = 7101
        private const val PAYMENT_REQUEST_CODE = 7102
